...
namespace Content
{
    ...
    {
        static Table<Str8, Graphic::VertexSemantic> kMapping
        {
            { "NORMAL",     Graphic::VertexSemantic::Normal    },
            { "POSITION",   Graphic::VertexSemantic::Position  },
            { "COLOR",      Graphic::VertexSemantic::Color0    },
        ...
        const auto Iterator = kMapping.find(Name);
    ...
    {
        Graphic::TextureAddress AddressModeU = Graphic::TextureAddress::Repeat;
        Graphic::TextureAddress AddressModeV = Graphic::TextureAddress::Repeat;
        Graphic::TextureFilter Filter        = Graphic::TextureFilter::Point;

        switch (GLTFSampler.wrapS)
        {
        case TINYGLTF_TEXTURE_WRAP_CLAMP_TO_EDGE:
            AddressModeU = Graphic::TextureAddress::Clamp;
            break;
        case TINYGLTF_TEXTURE_WRAP_MIRRORED_REPEAT:
            AddressModeU = Graphic::TextureAddress::Mirror;
            break;
        default:
            break;
        }
        ...
        {
        case TINYGLTF_TEXTURE_WRAP_CLAMP_TO_EDGE:
            AddressModeV = Graphic::TextureAddress::Clamp;
            break;
        case TINYGLTF_TEXTURE_WRAP_MIRRORED_REPEAT:
            AddressModeV = Graphic::TextureAddress::Mirror;
            break;
        default:
            break;
        }
    ...
    Tracker<Graphic::Texture> LoadTexture(Ref<tinygltf::Model> GLTFModel, ConstRef<tinygltf::Texture> GLTFTexture)
    {
        ...
        Blob Chunk = Blob::Bytes(GLTFImage.image.size());
    ...
    Bool GLTFLoader::Load(Ref<Service> Service, Ref<Scope> Scope, AnyRef<Blob> Data)
    {
        ...
        Blob   BlockForVertices  = Blob::Bytes(BytesForVertices);
        Blob   BlockForIndices   = Blob::Bytes(BytesForIndices);
        ...
        Vector<Tracker<Graphic::Material>> Materials { };
        for (ConstRef<tinygltf::Material> GLTFMaterial : GLTFModel.materials)
        {
            ...
            if (const SInt32 Index = GLTFMaterial.pbrMetallicRoughness.baseColorTexture.index; Index >= 0)
            {
            ...
            if (const SInt32 Index = GLTFMaterial.pbrMetallicRoughness.metallicRoughnessTexture.index; Index >= 0)
            {
                ConstRef<tinygltf::Texture> GLTFTexture = GLTFModel.textures[Index];
            ...
            if (const SInt32 Index = GLTFMaterial.normalTexture.index; Index >= 0)
            {
                ConstRef<tinygltf::Texture> GLTFTexture = GLTFModel.textures[Index];
            ...
            if (const SInt32 Index = GLTFMaterial.emissiveTexture.index; Index >= 0)
            {
                ConstRef<tinygltf::Texture> GLTFTexture = GLTFModel.textures[Index];
            ...
            if (const SInt32 Index = GLTFMaterial.occlusionTexture.index; Index >= 0)
            {
                ConstRef<tinygltf::Texture> GLTFTexture = GLTFModel.textures[Index];
        ...
        for (ConstRef<tinygltf::Mesh> GLTFMesh : GLTFModel.meshes)
        {
            ...
            // Parse material
            Render::Mesh::Primitive Primitive;
            ...
            if (GLTFPrimitive.indices >= 0)
            {
                ...
                Primitive.Indices = Render::Mesh::Attribute(Length, Offset, Stride);
            }
