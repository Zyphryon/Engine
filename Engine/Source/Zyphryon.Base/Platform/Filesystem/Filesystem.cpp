...
inline namespace Base
{
    ...
    Filesystem::Result Filesystem::Ensure(Text Path)
    {
        if (const SInt Separator = StrFindLast(Path, '/'); Separator > 0)
        {
            return Filesystem::MakeAll(Path.Slice(0, static_cast<UInt>(Separator)));
        }
