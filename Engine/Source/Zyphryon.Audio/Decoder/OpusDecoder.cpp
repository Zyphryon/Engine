...
namespace Audio
{
    ...
    OpusDecoder::OpusDecoder(ConstSpan<Byte> Samples, UInt32 Frequency, UInt16 Stride, UInt64 Frames)
        : mHandle       { nullptr },
    ...
    void OpusDecoder::Probe(Ptr<UInt32> Frequency, Ptr<UInt32> Stride, Ptr<UInt64> Frames) const
    {
    ...
    Bool OpusDecoder::Seek(UInt64 Frame)
    {
    ...
    UInt32 OpusDecoder::Expand(UInt32 Packet)
    {
        if (mStaged == Packet + 1)
        {
    ...
    UInt64 OpusDecoder::Read(Span<Real32> Output)
    {
        if (mHandle == nullptr)
        {
            return 0;
