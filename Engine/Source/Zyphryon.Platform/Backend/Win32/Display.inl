    ...
    void Display::Poll()
    {
        ...
        static constexpr auto OnEnumerate = [](HMONITOR Handle, HDC, LPRECT, LPARAM Context) -> BOOL
        {
            ...
            if (GetMonitorInfoW(Handle, AddressOf(Info)))
            {
                ...
                Monitor.SetName(StrConvertUTF8(Info.szDevice));
        ...
        for (ConstRef<DISPLAYCONFIG_PATH_INFO> Path : Paths)
        {
            ...
            const Str  Name = StrConvertUTF8(SourceName.viewGdiDeviceName);
            ...
            if (Slot >= 0)
            {
                ...
                {
                    Entry.SetName(StrConvertUTF8(TargetName.monitorFriendlyDeviceName));
                }
