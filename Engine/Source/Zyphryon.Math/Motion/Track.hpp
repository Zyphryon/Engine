...
#include "Zyphryon.Math/Curve/Bezier.hpp"
...
inline namespace Math
{
    ...
    public:
        ...
        ///
        /// \param Incoming The tangent arriving at each sample, one per sample.
        /// \param Outgoing The tangent leaving each sample, one per sample.
        /// \param Count    The number of samples.
        ZY_INLINE void SetTangents(ConstPtr<Type> Incoming, ConstPtr<Type> Outgoing, UInt Count)
        {
            ZY_ASSERT(Count == mValues.GetSize(), "A track needs one tangent pair for every sample");

            mIncoming.Resize(Count);
            mOutgoing.Resize(Count);

            if (Count > 0)
            {
                Blit(mIncoming.GetData(), Count * sizeof(Type), Incoming);
                Blit(mOutgoing.GetData(), Count * sizeof(Type), Outgoing);
            }
        }

        /// \brief Checks whether the track carries the tangents that \ref Interpolation::Hermite needs.
        ///
        /// \return `true` when the track carries one tangent pair per sample, otherwise `false`.
        ZY_INLINE Bool HasTangents() const
        {
            return mOutgoing.GetSize() == mValues.GetSize() && !mValues.IsEmpty();
        }

        /// \brief Gets the tangent arriving at each sample.
        ///
        /// \return A read-only view over the incoming tangents, or an empty view when the track has none.
        ZY_INLINE ConstSpan<Type> GetIncoming() const
        {
            return mIncoming;
        }

        /// \brief Gets the tangent leaving each sample.
        ///
        /// \return A read-only view over the outgoing tangents, or an empty view when the track has none.
        ZY_INLINE ConstSpan<Type> GetOutgoing() const
        {
            return mOutgoing;
        }
        ...
        ZY_INLINE Type Sample(ConstRef<Cursor> Where, ConstRef<Type> Fallback) const
        {
            ...
            // A spline mode is tried first, so a rotation track follows its curve instead of the default blend.
            if constexpr (IsSplinable<Type>)
            {
                if (mInterpolation == Interpolation::Hermite && HasTangents())
                {
                    const Real32 Step = Measure(Where);

                    const Type Start = mOutgoing[Where.Lower] * Step;
                    const Type End   = mIncoming[Where.Upper] * Step;
                    return Renormalize(AnyBezier<Type>::FromHermite(Previous, Next, Start, End).Point(Where.Delta));
                }
                ...
                if (mInterpolation == Interpolation::Cubic)
                {
                    ...
                    {
                        Before = Align(Previous, Before);
                        End    = Align(Previous, End);
                        After  = Align(End,      After);
                    }
                    return Renormalize(AnyBezier<Type>::FromCatmullRom(Before, Previous, End, After).Point(Where.Delta));
                }
            }
    ...
    private:
        ...
        /// \brief Restores the unit length that a spline blend does not preserve, for types that require it.
        ///
        /// \param Value The blended value.
        /// \return The value, renormalized when the type carries a unit-length invariant.
        ZY_INLINE static Type Renormalize(AnyRef<Type> Value)
        {
            if constexpr (IsSlerpable<Type>)
            {
        ...
        /// \brief Flips a value onto the anchor's hemisphere, so the pair describes the shorter arc.
        ///
        /// \param Anchor The value to match the hemisphere of.
        /// \param Value  The value to flip when it opposes \p Anchor.
        /// \return The value, negated when it lies on the opposite hemisphere.
        ZY_INLINE static Type Align(ConstRef<Type> Anchor, ConstRef<Type> Value)
        {
            return Type::Dot(Anchor, Value) < 0.0f ? -Value : Value;
        }

        /// \brief Measures the segment a cursor sits in, to bring per-second tangents onto the cursor's scale.
        ///
        /// \param Where The position within the track to measure at.
        /// \return The segment's duration in seconds.
        ZY_INLINE Real32 Measure(ConstRef<Cursor> Where) const
        {
            if (IsUniform())
            {
                return 1.0f / mRate;
            }
            return static_cast<Real32>(mTimes[Where.Upper] - mTimes[Where.Lower]);
        }
