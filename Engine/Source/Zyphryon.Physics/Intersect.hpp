...
namespace Physics
{
    ...
    /// \param Manifold The output parameter to store the contact manifold if an intersection occurs.
    ...
    /// \param Manifold The output parameter to store the contact manifold if an intersection occurs.
    /// \return `true` if the shapes intersect, `false` otherwise.
    ZY_INLINE Bool Intersects(Rect First, Circle Second, Ptr<Manifold> Manifold)
    {
