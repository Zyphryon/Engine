...
namespace Graphic
{
    ...
    /// \brief TODO_DOC
    ZY_INLINE Bool D3D11Check(HRESULT Result)
    {
        if (FAILED(Result))
        {
            ...
            LOG_ERROR("Driver: {0}", StrConvertUTF8(StrConvert(Message)));
            return false;
        }
        return true;
    }
    ...
    /// \brief TODO_DOC
    constexpr auto D3D11TranslateDSV(TextureFormat Value)
    {
        switch (Value)
        {
    ...
    /// \brief TODO_DOC
    constexpr auto D3D11TranslateSRV(TextureFormat Value)
    {
        switch (Value)
        {
        case TextureFormat::D16UIntNorm:
