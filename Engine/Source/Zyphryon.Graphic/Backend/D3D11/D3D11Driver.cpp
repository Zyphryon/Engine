...
#include "Zyphryon.Graphic/Backend/Metadata.hpp"
...
namespace Graphic
{
    ...
    Bool D3D11Driver::Initialize(Ptr<void> Output, ConstRef<Config> Config)
    {
        ...
        if (Successful)
        {
            ...
            if (FAILED(Result))
            {
                ...
                if (SUCCEEDED(Result))
                {
                    LOG_INFO("Driver: Enabling software mode (WARP)");
                }
            }
    ...
    void D3D11Driver::CreateBuffer(Object ID, Storage Storage, Usage Usage, UInt32 Capacity, ConstSpan<Byte> Data)
    {
        const D3D11_BUFFER_DESC Descriptor = CD3D11_BUFFER_DESC(
            Usage  == Usage::Uniform ? Align(Capacity, mDescription.Capabilities.UniformBlockAlignment) : Capacity,
            D3D11Convert(Usage),
            D3D11Convert(Storage),
            Storage == Storage::Dynamic ? D3D11_CPU_ACCESS_WRITE : 0);
    ...
    void D3D11Driver::UpdateBuffer(Object ID, UInt32 Offset, ConstSpan<Byte> Data)
    {
        ...
        const D3D11_BOX Destination = CD3D11_BOX(Offset, 0, 0, Offset + Data.GetSizeInBytes(), 1, 1);
    ...
    {
        constexpr D3D11_MAP      Mode = D3D11_MAP_WRITE_NO_OVERWRITE;
        D3D11_MAPPED_SUBRESOURCE Memory;

        if (D3D11Check(mDeviceImmediate->Map(mBuffers[ID].Get(), 0, Mode, 0, AddressOf(Memory))))
        {
    ...
    void D3D11Driver::CreateTexture(Object ID, TextureLayout Layout, TextureFormat Format, Storage Storage, Usage Usage, UInt16 Width, UInt16 Height, UInt8 Mipmaps, Multisample Samples, ConstSpan<Byte> Data)
    {
        ...
        Description.SampleDesc = mDeviceProperties.Multisample[Enum::Cast(Samples)];
        ...
        if (HasBit(Usage, Usage::Sample))
        {
            ...
            D3D11Check(mDevice->CreateShaderResourceView(Texture.Object.Get(), &View, Texture.Resource.GetAddressOf()));
        }
    }
    ...
    {
        static constexpr Command kDefaultStates { };

        for (UInt32 Batch = 0; Batch < Commands.GetSize(); ++Batch)
        {
            ConstRef<Command> NewestSubmission = Commands[Batch];
            ConstRef<Command> OldestSubmission = Batch > 0 ? Commands[Batch - 1] : kDefaultStates;
    ...
    void D3D11Driver::Commit(Object Pass)
    {
        ...
        if (Pass == kDisplay)
        {
            ...
            D3D11Check(mSwapchain->Present(0, Flag));
        }
    }
        ...
        for (UInt Index = 0; SUCCEEDED(mDeviceFactory->EnumAdapters1(Index, DXGIAdapter.GetAddressOf())); ++Index)
        {
            ...
            if (SUCCEEDED(DXGIAdapter->GetDesc1(&DXGIDescription)))
            {
                if ((DXGIDescription.Flags & DXGI_ADAPTER_FLAG_SOFTWARE) == 0)
                {
                    ...
                    AdapterInfo.Description = StrConvertUTF8(StrConvert(DXGIDescription.Description));
            ...
            if (ComPtr<IDXGIOutput> DXGIOutput; SUCCEEDED(DXGIAdapter->EnumOutputs(0, DXGIOutput.GetAddressOf())))
            {
                ...
                if (SUCCEEDED(DXGIOutput->GetDisplayModeList(Format, 0, &Length, nullptr)))
                {
                    ...
                    D3D11Check(DXGIOutput->GetDisplayModeList(Format, 0, &Length, Descriptions.GetData()));
    ...
    void D3D11Driver::LoadCapabilities()
    {
        ...
        // Defines the UBO requirements by D3D11.
        ...
        for (UInt32 Level = 1, Last = 1; Level <= 8; ++Level)
        {
            UInt32 Quality = 0;

            // TODO Check other formats
            const HRESULT Result = mDevice->CheckMultisampleQualityLevels(DXGI_FORMAT_R8G8B8A8_UNORM, Level, AddressOf(Quality));

            if (SUCCEEDED(Result) && Quality > 0)
            {
                mDeviceProperties.Multisample[Level] = DXGI_SAMPLE_DESC { Level, Quality - 1 };
            }
            else
            {
                mDeviceProperties.Multisample[Level] = mDeviceProperties.Multisample[Last];
            }
        }
    }
    ...
    {
        const UINT        Flags  = (mDeviceProperties.Tearing ? DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING : 0);
        const DXGI_FORMAT Format = D3D11Convert(Config.ColorFormat);

        DXGI_SWAP_CHAIN_DESC Description { };
        Description.BufferCount       = kMaxFrames;
        Description.BufferUsage       = DXGI_USAGE_RENDER_TARGET_OUTPUT;
        Description.Flags             = DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH | Flags;
        Description.BufferDesc.Format = Format == DXGI_FORMAT_R8G8B8A8_UNORM_SRGB ? DXGI_FORMAT_R8G8B8A8_UNORM : Format;
    ...
    void D3D11Driver::CreateSwapchainResources(Ref<D3D11Pass> Pass, ConstRef<Config> Config) const
    {
        ...
        if (Config.DepthFormat != TextureFormat::Unspecified)
        {
            ...
            D3D11Check(mDevice->CreateDepthStencilView(Depth.Get(), nullptr, Pass.DepthStencil.Target.GetAddressOf()));
        }
    }
