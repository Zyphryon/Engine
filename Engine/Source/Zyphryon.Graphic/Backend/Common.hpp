...
namespace Graphic
{
    ...
    enum : UInt32
    {
        ...
        /// \brief The maximum number of mipmap levels for a texture.
        kMaxMipmaps     = 0x0010,

        /// \brief The maximum number of render pass resources.
        kMaxPasses      = 0x0040,
    ...
    ZY_DEFINE_BITWISE_ENUM(Channel)
    ...
    {
        None,   ///< No Shading language.
        HLSL,   ///< High-Level Shading Language used primarily with Direct3D.
        GLSL,   ///< OpenGL Shading Language used for OpenGL pipelines.
    };
    ...
    /// \brief Specifies the intended storage pattern of a GPU resource in terms of CPU and GPU access.
    ...
    enum class TextureFormat : UInt8
    {
        ...
        ETC2RGBUIntNorm,            ///< Block-compressed RGB (ETC2), unsigned normalized.
        ETC2RGBUIntNorm_sRGB,       ///< Block-compressed RGB (ETC2), sRGB gamma-corrected.
        ETC2RGBA1UIntNorm,          ///< Block-compressed RGB (ETC2) with 1-bit punch-through alpha, unsigned normalized.
        ETC2RGBA1UIntNorm_sRGB,     ///< Block-compressed RGB (ETC2) with 1-bit punch-through alpha, sRGB gamma-corrected.
        ETC2RGBAUIntNorm,           ///< Block-compressed RGBA (ETC2/EAC), unsigned normalized.
        ETC2RGBAUIntNorm_sRGB,      ///< Block-compressed RGBA (ETC2/EAC), sRGB gamma-corrected.
        EACR11UIntNorm,             ///< Block-compressed single-channel (red, EAC), unsigned normalized.
        EACR11SIntNorm,             ///< Block-compressed single-channel (red, EAC), signed normalized.
        EACRG11UIntNorm,            ///< Block-compressed two-channel (red-green, EAC), unsigned normalized.
        EACRG11SIntNorm,            ///< Block-compressed two-channel (red-green, EAC), signed normalized.
    ...
    enum class UniformType : UInt8
    {
        ...
        IntColor8,  ///< Color value (RGBA integer components).
        Float,      ///< Single 32-bit floating-point value.
        Float2,     ///< Two-component 32-bit floating-point vector.
        Float3,     ///< Three-component 32-bit floating-point vector.
        Float4,     ///< Four-component 32-bit floating-point vector.
        Int,        ///< Single 32-bit signed integer value.
        Int2,       ///< Two-component 32-bit signed integer vector.
        Int3,       ///< Three-component 32-bit signed integer vector.
        Int4,       ///< Four-component 32-bit signed integer vector.
        UInt,       ///< Single 32-bit unsigned integer value.
        UInt2,      ///< Two-component 32-bit unsigned integer vector.
        UInt3,      ///< Three-component 32-bit unsigned integer vector.
        UInt4,      ///< Four-component 32-bit unsigned integer vector.
    };
    ...
    /// \brief Describes the logical buffer slots for a vertex buffer in a shader.
    enum class VertexSlot : UInt8
    {
    ...
    using Parameter = Variant<
        ...
        Real32,     Vector2,        Vector3,        Array<Real32, 4>,
        SInt32,     IntVector2,     IntVector3,     Array<SInt32, 4>,
        UInt32,     UIntVector2,    UIntVector3,    Array<UInt32, 4>
    >;
    ...
    struct Adapter final
    {
        ...
        Sequence<Resolution> Resolutions;
    };
    ...
    struct Config final
    {
        ...
        /// The depth format used by the graphics device.
    ...
    struct Description final
    {
        ...
        /// The shader model supported by the graphics device.
        Tier              Tier         = Tier::Level1;

        /// The shader language supported by the driver.
    ...
    struct Sampler final
    {
        ...
        /// \brief Checks if the sampler configuration is the default configuration.
        ///
        /// \return `true` if the sampler is in its default state, `false` otherwise.
        ZY_INLINE Bool IsDefault() const
        {
            return AddressModeU == TextureAddress::Clamp &&
    ...
    /// \brief Represents a preprocessor macro used in shader compilation.
    struct Macro final
    {
        /// The name of the preprocessor macro.
        Str32 Name;

        /// The value assigned to the preprocessor macro (optional).
        Str32 Value;

        /// \brief Default constructor for the Macro struct.
        ZY_INLINE constexpr Macro(Text Name, Text Value)
            : Name  { Name  },
              Value { Value }
        {
        }

        /// \brief Copy constructor for the Macro struct.
        ZY_INLINE constexpr Macro(ConstRef<Macro> Other)
            : Name  { '\0' },
              Value { '\0' }
        {
            Name  = Other.Name;
    ...
    struct Command final
    {
        ...
        Sequence<Stream, kMaxVertices>  Vertices;
