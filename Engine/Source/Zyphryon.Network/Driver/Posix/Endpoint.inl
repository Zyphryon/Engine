...
namespace Network
{
    ...
    Bool Endpoint::Resolve(ConstRef<Address> Address, Transport Transport, Ref<Endpoint> Output)
    {
        ...
        const ConstPtr<Char> Name = Address.IsAny() ? nullptr : Address.Host.GetData();

        if (const SInt32 Code = getaddrinfo(Name, Service.GetData(), AddressOf(Hint), AddressOf(List)))
        {
            LOG_E("Network: '{0}' could not be resolved ({1})", Address.Host, gai_strerror(Code));
            return false;
        }
