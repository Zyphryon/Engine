...
namespace Physics
{
    ...
    public:
        ...
        {
            SetNormal(AnyVector3(Normal.GetX(), Normal.GetY(), static_cast<Type>(0)));
        }
        ...
        ZYPHRYON_INLINE void AddContact(AnyVector2<Type> Point)
        {
            AddContact(AnyVector3<Type>(Point.GetX(), Point.GetY(), static_cast<Type>(0)));
        }

        /// \brief Adds a 3D contact point to the manifold.
        ///
        /// \param Point The 3D contact point to add.
        ZYPHRYON_INLINE void AddContact(AnyVector3<Type> Point)
        {
            mContacts.push_back(Point);
        }

        /// \brief Gets the list of contact points in the manifold.
        ///
        /// \return A constant span containing the contact points.
        ZYPHRYON_INLINE ConstSpan<AnyVector3<Type>> GetContacts() const
        {
            return mContacts;
        }
    ...
    private:
        ...
        Type                                          mPenetration;
        Vector<AnyVector3<Type>, kMaxCollisionPoints> mContacts;
    };
