...
namespace Tool::Baker::Texture
{
        ...
        Ptr<Byte>      Target, UInt32 TargetWidth, UInt32 TargetHeight)
    {
        for (UInt32 Y = 0; Y < TargetHeight; ++Y)
        {
            ...
            for (UInt32 X = 0; X < TargetWidth; ++X)
            {
                ...
                const Color P00 = Fetch<Type, Channels, SRGB>(Source, Y0 * SourceWidth + X0);
                const Color P01 = Fetch<Type, Channels, SRGB>(Source, Y0 * SourceWidth + X1);
                const Color P10 = Fetch<Type, Channels, SRGB>(Source, Y1 * SourceWidth + X0);
                const Color P11 = Fetch<Type, Channels, SRGB>(Source, Y1 * SourceWidth + X1);
    ...
    using Reducer = void (*)(ConstPtr<Byte>, UInt32, UInt32, Ptr<Byte>, UInt32, UInt32);

    template<Component Value>
    static constexpr Reducer kReducers[kMaxChannels][2] =
    {
        { Reduce<StorageOf<Value>, 1, false>, Reduce<StorageOf<Value>, 1, true> },
        { Reduce<StorageOf<Value>, 2, false>, Reduce<StorageOf<Value>, 2, true> },
        { Reduce<StorageOf<Value>, 3, false>, Reduce<StorageOf<Value>, 3, true> },
        { Reduce<StorageOf<Value>, 4, false>, Reduce<StorageOf<Value>, 4, true> },
    };

    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    static Reducer Pick(ConstRef<Graphic::TextureMetadata> Format)
    {
        const UInt32 Channel = Clamp<UInt32>(Format.Components, 1, kMaxChannels) - 1;
        const UInt32 Space   = Format.IsSRGB ? 1 : 0;

        switch (GetComponent(Format))
        {
        case Component::SInt8:
            return kReducers<Component::SInt8>[Channel][Space];
        case Component::UInt16:
            return kReducers<Component::UInt16>[Channel][Space];
        case Component::SInt16:
            return kReducers<Component::SInt16>[Channel][Space];
        case Component::Half:
            return kReducers<Component::Half>[Channel][Space];
        case Component::Real32:
            return kReducers<Component::Real32>[Channel][Space];
        default:
            return kReducers<Component::UInt8>[Channel][Space];
        }
    }
    ...
    Bitmap Mipmapper::Generate(AnyRef<Bitmap> Source, UInt8 Levels)
    {
        ...
        const Reducer Downsample = Pick(Metadata);
